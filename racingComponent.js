...
function unloadRacingComponent() {
  try {
    ...
    for ( var i=0; i<wpTableList.length; i++ )
      wpTableList[i] = null;
    wpTableList = null;
  }
  catch ( ex ) { }
...
function isAllReserve(horseStats) {
  ...
  for ( var i=0; i<horseStats.length; i++ ) {
    if ( horseStats[i].indexOf('S') >= 0 )
      sCount++;
    else if ( horseStats[i].indexOf('R') >= 0 )
      rCount++;
  }
  if ( rCount==horseStats.length )
    return true;
  if ( rCount > 0 && rCount + sCount == horseStats.length )
    return true;
  return false;
}
...
function isBracketReserveOrScratch(tbObj) {
  ...
  for ( var i=0; i<tbObj.length; i++ ) {
    if ( tbObj[i].hStat.indexOf('S') >= 0 )
      sCount++;
    else if ( tbObj[i].hStat.indexOf('R') >= 0 )
      rCount++;
  }
  if ( rCount==tbObj.length )
    return true;
  if ( rCount > 0 && rCount + sCount == tbObj.length )
    return true;
  return false;
}
...
function RacingWPTable(rNo) {
  ...
  this.createBetObject = function() {
    ...
    for ( var i=0; i<this.selectLeg.length; i++ ) {
      if ( this.selectBanker[i]==1 ) {
        if ( betObj.bankerNo > 0 )
          betObj.bankerBuf.append('+');
        betObj.bankerBuf.append(i);
        betObj.bankerNo++;
      }
      if ( this.selectLeg[i]==1 ) {
        if ( betObj.legNo > 0 )
          betObj.legBuf.append('+');
  ...
  this.updateRowHeights = function() {      
      ...
      if (tb1 != null) {
          ...
          for (var i = 0; i < trs1.length; i++)
              this.rowHeights[i] = trs1[i].offsetHeight;          
      }
  }
  ...
  this.setInnerTableHtml = function() {
      ...
      try { syncTableRowHeight(); } catch (e) { }
  ...
  this.generateTable = function() {
    ...
    if ( this.dropdownValue!=null && this.dropdownValue.length > 0 ) {
      ...
      buf.append('<select onchange="')
         .append('poolBoxChange(\'').append(this.divId)
         .append('\', ').append(this.tableId).append(', this.value);" class="select2" style="WIDTH:100px">');
  ...
  this.generateInnerTable = function() {
      ...
      if (this.firstLeg > 0 && this.firstLeg != this.raceNo)
          this.sellStatus = wpTableList[this.firstLeg].sellStatus;
      ...
      buf.append('<td class="tableContentHeadAlup" style="BORDER-BOTTOM:#cccccc 1px solid" nowrap align="center">').append(this.lblHorseNum).append('</td>');
      if (enableBS) {
          if (this.enableBanker)
              buf.append('<td class="tableContentHeadAlup" style="BORDER-BOTTOM:#cccccc 1px solid" align="center" nowrap>').append(this.lblBanker).append('</td>');
          buf.append('<td class="tableContentHeadAlup" style="BORDER-BOTTOM:#cccccc 1px solid" align="center" nowrap>').append(this.lblLeg).append('</td>');
      }
      if (this.showHorseName)
      buf.append('<td class="tableContentHeadAlup" style="BORDER-BOTTOM:#cccccc 1px solid" nowrap>').append(this.lblHorseName).append('</td>');
      if (this.showOdd) {
      ...
      if (this.enablePla)
          buf.append('<td class="tableContentHeadAlup" style="BORDER-BOTTOM:#cccccc 1px solid" align="center" nowrap>').append(this.lblPla).append('</td>');
      }
      buf.append('</tr>');
      ...
      for (var i = 1; i <= this.fieldSize; i++) {
          ...
          // horse num
          if (this.showHorseNum) {
              buf.append('<td align="center" class="').append(tbStyle[i % 2]).append(" info_cell").append('" style="padding-left:1px;padding-right:1px;line-height:16px;')
         .append(this.enableBorder ? 'border-left:1px solid #CCCCCC' : '').append('"><nobr>').append(i);
          ...
          if (enableBS) {
              // banker box
              if (this.enableBanker) {
                  var disabled = this.tableObj[i][0].hName == '' || this.tableObj[i][0].hScr == '1' || ranRace >= this.firstLeg
                      || this.startSell == 0 || this.sellStatus.indexOf(this.selectPool) < 0 || isBracketReserveOrScratch(this.tableObj[i]);
                  if (disabled)
                      this.selectBanker[i] = 0;
                  ...
                  buf.append('>');
                  buf.append('<input type="checkbox" class="checkbox" onclick="bankerClick(\'').append(this.divId)
             .append('\', ').append(this.tableId).append(', ').append(i).append(');" ');
              ...
              // leg box
              var disabled = this.selectLegField == 1 || this.tableObj[i][0].hName == '' || this.tableObj[i][0].hScr == '1' || ranRace >= this.firstLeg
                    || this.startSell == 0 || this.sellStatus.indexOf(this.selectPool) < 0 || isBracketReserveOrScratch(this.tableObj[i]);
              if (disabled)
                  this.selectLeg[i] = 0;
              buf.append('<td class="').append(tbStyle[i % 2]).append(" legCheckbox").append('" align="center"');
              buf.append((this.selectLeg[i] == 1) ? ' style="background-color: #FFF4B0"' : '');
              buf.append('>');
              buf.append('<input type="checkbox" class="checkbox" onclick="legClick(\'').append(this.divId)
             .append('\', ').append(this.tableId).append(', ').append(i).append(');" ')
          ...
          if (this.showHorseName) {
              ...
              if (this.horseNameWidthUnbound)
                buf.append('" ><div class="divWinHorseName0">');
              else if(this.enableBanker)
                buf.append('" ><div class="divWinHorseName2">');
              else                
                buf.append('" ><div class="divWinHorseName1">');
          ...
          for (var j = 0; j < this.tableObj[i].length; j++) {
              ...
              if (isOverseaMeeting && this.tableObj[i][j].hCode != '')
                  buf.append('(').append(this.tableObj[i][j].hCode).append(')');

              if (this.tableObj[i][j].hName != '')
                  buf.append('<span class="hkscsfontfamily">').append(this.tableObj[i][j].hName).append('</span>');
              else
                  buf.append('-');
              ...
              if (!isAllBracketReserve(this.tableObj[i]) && this.tableObj[i][j].hStat.indexOf('R') >= 0)
                  buf.append('<nobr>(').append(reserveLbl).append(')</nobr>');
              if (isOverseaMeeting && this.tableObj[i][j].hStat.indexOf('S') >= 0)
         ...
         .append((this.enableBorder && !this.enablePla) ? 'border-right:1px solid #CCCCCC' : '').append('">');
          if (this.winOdds[i] == 'SCR' || this.tableObj[i][0].hScr == '1')
              buf.append('<nobr>').append(scratchLbl).append('</nobr>')
          else if (isBracketReserveOrScratch(this.tableObj[i]))
              buf.append('<span class="wpTdColor"><nobr>---</nobr></span>');
          else if (this.winOdds[i] != null && ranRace < this.firstLeg && this.startSell == 1 && this.sellStatus.indexOf('WIN') >= 0)
              buf.append('<a class="wpTdColor" style="color:')
             .append(getOddsFgColor(this.winColorInd[i])).append(';background-color:')
             .append(getOddsBgColor(this.winColorInd[i]))
             .append('" href="javascript:processQuickBet(\'WIN\', \'' + this.raceNo + '\', ' + i + ')">' + this.winOdds[i] + '</a>');
          else
             ...
             //test
             .append('<span style="background-color: red; display: block; height: 5px; width: 20px"></span>');
             //test
          buf.append('</td>');
          ...
          if (this.enablePla) {
              buf.append('<td class="').append(tbStyle[i % 2]).append(" info_cell").append('" align="center" style="')
           .append(this.enableBorder ? 'border-right:1px solid #CCCCCC' : '').append('">');
              if (this.plaOdds[i] == 'SCR' || this.tableObj[i][0].hScr == '1')
                  buf.append('<nobr>').append(scratchLbl).append('</nobr>');
              else if (isBracketReserveOrScratch(this.tableObj[i]))
                  buf.append('<span class="wpTdColor"><nobr>---</nobr></span>');
              else if (this.plaOdds[i] != null && ranRace < this.firstLeg && this.startSell == 1 && this.sellStatus.indexOf('PLA') >= 0)
                  buf.append('<a class="wpTdColor" style="color:')
             .append(getOddsFgColor(this.plaColorInd[i])).append(';background-color:')
             .append(getOddsBgColor(this.plaColorInd[i]))
             .append('" href="javascript:processQuickBet(\'PLA\', \'' + this.raceNo + '\', ' + i + ')">' + this.plaOdds[i] + '</a>');
              else
                  buf.append('<span class="wpTdColor" style="color:')
             .append(getOddsFgColor(this.plaColorInd[i])).append(';background-color:')
             .append(getOddsBgColor(this.plaColorInd[i]))
             .append('">').append(this.plaOdds[i]).append('&nbsp</span>');
              buf.append('</td>');
          }
          }
      ...
      for (var i = this.fieldSize + 1; i <= this.tableSize; i++) {
         ...
         .append(this.enableBorder ? 'border-left:1px solid #CCCCCC' : '').append('">&nbsp;</td>');
         ...
         .append((this.enableBorder && !this.enablePla) ? 'border-right:1px solid #CCCCCC' : '').append('">&nbsp;</td>');
          }

          if (this.showOdd) {
          if (this.enablePla) {
              buf.append('<td class="').append(tbStyle[i % 2]).append('" style="')
           .append(this.enableBorder ? 'border-right:1px solid #CCCCCC' : '').append('">&nbsp;/td>');
          }
          }
      ...
      if (enableBS && (this.enableField || this.force2ShowFieldRow) && this.fieldSize > 1) {
          ...
          buf.append('<tr>');
          if (this.showHorseNum)
          buf.append('<td align="center" class="tableContentEnd" style="BORDER-TOP:#a6a6a6 2px solid">F</td>');
          if (this.enableBanker)
              buf.append('<td class="tableContentEnd" style="BORDER-TOP:#a6a6a6 2px solid"align="center">&nbsp;</td>');
          buf.append('<td class="tableContentEnd" style="BORDER-TOP:#a6a6a6 2px solid" align="center"');
          buf.append((this.selectLegField == 1) ? ' style="background-color: #FFF4B0"' : '');
          buf.append('>');
          if (this.enableField) {
          buf.append('<input type="checkbox" class="checkbox" onclick="fieldClick(\'')
         .append(this.divId).append('\', ').append(this.tableId).append(');" ');
          ...
          buf.append('</td>');
          if (this.showHorseName)
          buf.append('<td class="tableContentEnd" style="BORDER-TOP:#a6a6a6 2px solid">').append(this.lblField).append('</td>');
          if (this.showOdd) {
          buf.append('<td class="tableContentEnd" style="BORDER-TOP:#a6a6a6 2px solid" align="center">&nbsp;</td>');
          if (this.enablePla)
              buf.append('<td class="tableContentEnd" style="BORDER-TOP:#a6a6a6 2px solid" align="center">&nbsp;</td>');
...
function fieldClick(divObj, tblId) {
  if ( wpTableList[tblId].selectLegField!=1 ) {
    ...
    wpTableList[tblId].selectLegField = 0;
...
function WinPlaceCheckBoxObj(rNo) {
  ...
  this.generateTable = function() {
    ...
    for ( var i=1; i<=this.tableSize; i++ ) {
      ...
      buf.append('<tr height="22px"><td class="').append(tbStyleDtls[i%2]).append(" info_cell")
         .append('" valign="middle" align="center" rowspan="').append(subRowSize).append('">')
         .append(i).append('</td>');

      buf.append(getTableInnerRow(this.tableObj[i][0], '', true, true));

      buf.append('<td id="winCell').append(i).append('" class="').append(tbStyleDtls[i%2])
         .append('" valign="middle" rowspan="').append(subRowSize).append('"></td>');
      buf.append('<td id="plaCell').append(i).append('" class="').append(tbStyleDtls[i%2])
         .append('" valign="middle" rowspan="').append(subRowSize).append('"></td>');
      buf.append('<td id="wipCell').append(i).append('" class="').append(tbStyleDtls[i%2])
         .append('" valign="middle" align="center" rowspan="').append(subRowSize).append('"></td></tr>');
  ...
  this.setInnerTableHtml = function() {
    for (var i = 1; i <= this.tableSize; i++) {
      ...
      var buf = new StringBuffer();
      if ( enableBS ) {
        var disabled = this.tableObj[i][0].hName=='' || this.selectField['WIN']==1 || this.tableObj[i][0].hScr=='1' || ranRace>=this.raceNo
                    || this.startSell==0 || this.sellStatus.indexOf('WIN')<0 || isBracketReserveOrScratch(this.tableObj[i]);
        if ( disabled )
          this.selectLeg['WIN'][i] = 0;
        buf.append('<input style="vertical-align:middle" type="checkbox" class="checkbox" onclick="wpLegClick(')
           .append(this.raceNo).append(', ').append(i).append(', \'WIN\');" ');
      ...
      if ( this.winOdds[i]=='SCR' || this.tableObj[i][0].hScr=='1' )
        ...
        buf.append('<span class="wpTdColor"><nobr>---</nobr></span>');
      else if ( this.winOdds[i]!=null && ranRace<this.raceNo && this.startSell==1 && this.sellStatus.indexOf('WIN')>=0 )
        buf.append('<a class="wpTdColor" style="color:')
             .append(getOddsFgColor(this.winColorInd[i])).append(';background-color:')
             .append(getOddsBgColor(this.winColorInd[i]))
             .append('" href="javascript:processQuickBet(\'WIN\', \'' + this.raceNo + '\', ' + i + ')">' + this.winOdds[i] + '</a>');
        ...
        buf.append('<span class="wpTdColor" style="color:')
             .append(getOddsFgColor(this.winColorInd[i])).append(';background-color:')
             .append(getOddsBgColor(this.winColorInd[i]))
             .append('">').append(this.winOdds[i]).append('</span>');
      ...
      if ( enableBS ) {
        var disabled = this.tableObj[i][0].hName=='' || this.selectField['PLA']==1 || this.tableObj[i][0].hScr=='1' || ranRace>=this.raceNo
                    || this.startSell==0 || this.sellStatus.indexOf('PLA')<0 || isBracketReserveOrScratch(this.tableObj[i]);
        if ( disabled )
          this.selectLeg['PLA'][i] = 0;
      buf.append('<input style="vertical-align:middle"  type="checkbox" class="checkbox" onclick="wpLegClick(')
           .append(this.raceNo).append(', ').append(i).append(', \'PLA\');" ');
      ...
      if ( this.plaOdds[i]=='SCR' || this.tableObj[i][0].hScr=='1' )
        ...
        buf.append('<span class="wpTdColor"><nobr>---</nobr></span>');
      else if ( this.plaOdds[i]!=null && ranRace<this.raceNo && this.startSell==1 && this.sellStatus.indexOf('PLA')>=0 )
        buf.append('<a class="wpTdColor" style="color:')
           .append(getOddsFgColor(this.plaColorInd[i])).append(';background-color:')
           .append(getOddsBgColor(this.plaColorInd[i]))
           .append('" href="javascript:processQuickBet(\'PLA\', \'' + this.raceNo + '\', ' + i + ')">' + this.plaOdds[i] + '</a>');
        ...
        buf.append('<span class="wpTdColor" style="color:')
           .append(getOddsFgColor(this.plaColorInd[i])).append(';background-color:')
           .append(getOddsBgColor(this.plaColorInd[i]))
           .append('">').append(this.plaOdds[i]).append('</span>');
      ...
      buf = new StringBuffer();
      if ( enableBS ) {
        var disabled = this.tableObj[i][0].hName=='' || this.selectField['W-P']==1 || this.tableObj[i][0].hScr=='1' || ranRace>=this.raceNo
                    || this.startSell==0 || this.sellStatus.indexOf('W-P')<0 || isBracketReserveOrScratch(this.tableObj[i]);
        if ( disabled )
          this.selectLeg['W-P'][i] = 0;
        buf.append('<input type="checkbox" class="checkbox" onclick="wpLegClick(')
           .append(this.raceNo).append(', ').append(i).append(', \'W-P\');" ');
    ...
    if ( enableBS ) {    
      ...
      var buf = new StringBuffer();
      buf.append('<input type="checkbox" class="checkbox" onclick="wpFieldClick(\'')
         .append(this.raceNo).append('\', \'WIN\');" ');
...
function setClothOrReserveCell(tbObj) {
  var buf = new StringBuffer();
  if (tbObj.hStat.indexOf('R') >= 0)
    buf.append('(').append(reserveLbl).append(')');
  else {
    buf.append('<img title="').append(tbObj.brand)
       .append('" width="17" height="22" id="wpTableImg').append(tbObj.brand)
       .append('" src="/info/include/images/spacer.gif">');
  }
  return buf.toString();
}
...
function getTableInnerRow(tbObj, pBorder, showColor, showSCR) {
  ...
  if ( showColor ) {
    ...
    if (tbObj.brand != '')
      buf.append(' id="wpColorCell').append(tbObj.brand).append('"');
    buf.append('>');
  ...
  if ( isOverseaMeeting ) {
    if ( tbObj.hCode!='' )
      buf.append('(').append(tbObj.hCode).append(')')
    if ( tbObj.hName!='' )
      buf.append('<span class="hkscsfontfamily">').append(tbObj.hName).append('</span>');
    else
      buf.append('-');
  }
  else
	...
	buf.append('<a href="javascript:WACommonTagging(\'horse\');goHorseRecord2(\'').append(jcewUrlLbl).append('\', \'').append(tbObj.hCode).append('\');">')
       .append('<span class="hkscsfontfamily">').append(tbObj.hName).append('</span>').append('</a>');
	// END Nielsen Online SiteCensus
  ...
  // Scratch or not
  if ( showSCR && tbObj.hStat.indexOf('S') >= 0 ) {
      buf.append('<td class="').append(tbStyleDtls[tbObj.num % 2]).append(" info_cell")
       .append('" align="center" colspan="4" ').append(pBorder).append('>(')
       .append(scratchLbl).append(')</td>');
  }
  else {
    // Draw & Handicap
    ...
    buf.append('<td class="').append(tbStyleDtls[tbObj.num % 2]).append(' info_cell" ').append(pBorder).append('>')
    if ( isOverseaMeeting )
      buf.append('<span class="hkscsfontfamily">').append(tbObj.jName).append('</span>');
    else
	  // START Nielsen Online SiteCensus
      //buf.append('<a href="javascript:goJockeyRecord2(\'').append(jcewUrlLbl).append('\', \'').append(tbObj.jCode).append('\');">')
	  buf.append('<a href="javascript:WACommonTagging(\'jockey\');goJockeyRecord2(\'').append(jcewUrlLbl).append('\', \'').append(tbObj.jCode).append('\');">')
         .append('<span class="hkscsfontfamily">').append(tbObj.jName).append('</span>').append('</a>');
	  // END Nielsen Online SiteCensus
    buf.append('</td>');
    ...
    buf.append('<td class="').append(tbStyleDtls[tbObj.num % 2]).append(' info_cell" ').append(pBorder).append('>')
    if ( isOverseaMeeting )
      buf.append('<span class="hkscsfontfamily">').append(tbObj.tName).append('</span>');
    else
	  // START Nielsen Online SiteCensus
      //buf.append('<a href="javascript:goTrainerRecord2(\'').append(jcewUrlLbl).append('\', \'').append(tbObj.tCode).append('\');">')
	  buf.append('<a href="javascript:WACommonTagging(\'trainer\');goTrainerRecord2(\'').append(jcewUrlLbl).append('\', \'').append(tbObj.tCode).append('\');">')
         .append('<span class="hkscsfontfamily">').append(tbObj.tName).append('</span>').append('</a>');
	  // END Nielsen Online SiteCensus
    buf.append('</td>');
  }
  return buf.toString();
...
function wpLegClick(tblId, i, pool) {
  if ( wpTableList[tblId].selectLeg[pool][i]!=1 )
    wpTableList[tblId].selectLeg[pool][i] = 1;
  else
...
function wpFieldClick(tblId, pool) {
  if ( wpTableList[tblId].selectField[pool]!=1 ) {
    ...
    wpTableList[tblId].selectField[pool] = 0;
    wpTableList[tblId].setInnerTableHtml();
...
function PWinTableObj(rNo) {
  ...
  this.generateTable = function() {
    ...
    for ( var i=1; i<=this.tableSize; i++ ) {
      ...
      buf.append('<tr height="22px"><td class="').append(tbStyleDtls[i%2])
         .append('" valign="middle" align="center" rowspan="').append(subRowSize).append('">')
         .append(i).append('</td>');

      buf.append(getTableInnerRow(this.tableObj[i][0], '', true, true));

      buf.append('<td id="pwin0Cell').append(i).append('" class="').append(tbStyleDtls[i%2])
         .append('" align="center" valign="middle" rowspan="').append(subRowSize).append('"></td>');
      buf.append('<td id="pwin1Cell').append(i).append('" class="').append(tbStyleDtls[i%2])
         .append('" align="center" valign="middle" rowspan="').append(subRowSize).append('"></td>');
      buf.append('<td id="pwin2Cell').append(i).append('" class="').append(tbStyleDtls[i%2])
         .append('" align="center" valign="middle" rowspan="').append(subRowSize).append('"></td>');
      buf.append('<td id="plaCell').append(i).append('" class="').append(tbStyleDtls[i%2])
         .append('" align="center" valign="middle" rowspan="').append(subRowSize).append('"></td></tr>');
  ...
  this.setInnerTableHtml = function() {
    for (var i = 1; i <= this.tableSize; i++) {
      ...
      // pwin odds
      if ( this.pwinOdds[0][i-1]=='SCR' || this.winOdds[i]=='SCR' || this.tableObj[i][0].hScr=='1' )
        document.getElementById('pwin0Cell'+i).innerHTML = scratchLbl;
      else
        document.getElementById('pwin0Cell'+i).innerHTML = this.pwinOdds[0][i-1];

      if ( this.pwinOdds[1][i-1]=='SCR' || this.winOdds[i]=='SCR' || this.tableObj[i][0].hScr=='1' )
        document.getElementById('pwin1Cell'+i).innerHTML = scratchLbl;
      else
        document.getElementById('pwin1Cell'+i).innerHTML = this.pwinOdds[1][i-1];
...
function JTComboChildTable(rNo) {
  ...
  this.generateTable = function(styleIdx) {
    ...
    for ( var i=1; i<=this.tableSize; i++ ) {
      ...
      buf.append('<tr height="22px"><td class="').append(tbStyleDtls[styleIdx%2])
         .append('" valign="middle" align="center" rowspan="').append(subRowSize).append('">')
         .append(this.raceNo).append('</td>');

      buf.append('<td class="').append(tbStyleDtls[styleIdx%2])
         .append('" valign="middle" align="center" rowspan="').append(subRowSize).append('">')
         .append(i).append('</td>');
      ...
      buf.append('<td id="winCell').append(this.raceNo).append('_').append(i).append('" class="').append(tbStyleDtls[styleIdx%2])
         .append('" valign="middle" rowspan="').append(subRowSize).append('"></td>');
      buf.append('<td id="plaCell').append(this.raceNo).append('_').append(i).append('" class="').append(tbStyleDtls[styleIdx%2])
         .append('" valign="middle" rowspan="').append(subRowSize).append('"></td>');
      buf.append('<td id="wipCell').append(this.raceNo).append('_').append(i).append('" class="').append(tbStyleDtls[styleIdx%2])
         .append('" valign="middle" align="center" rowspan="').append(subRowSize).append('"></td></tr>');
    ...
    if ( this.displayCnt() > 0 )
      buf.append('<tr><td colspan="10" bgcolor="#CCCCCC"><img src="/info/include/images/spacer.gif" width="1" height="1"></td></tr>');
  ...
  this.setInnerTableHtml = function() {
    for ( var i=1; i<=this.tableSize; i++ ) {
      ...
      if ( enableBS ) {
        ...
        if ( disabled )
          this.selectLeg['WIN'][i] = 0;
        buf.append('<input style="vertical-align:middle" type="checkbox" class="checkbox" onclick="mExcludeChkbox(')
            .append(this.raceNo).append(', ').append(i).append(', \'WIN\');" ');
        buf.append((this.selectLeg['WIN'][i]==1) ? 'checked ' : '');
        buf.append(disabled ? 'disabled' : '').append('>');
      }
      ...
      if ( this.plaOdds[i]=='SCR' || this.tableObj[i][0].hScr=='1' )
        buf.append('<nobr>').append(scratchLbl).append('<nobr>');
      else if ( this.plaOdds[i]!=null && this.startSell==1 && this.sellStatus.indexOf('PLA')>=0 )
        buf.append('<a class="wpTdColor" style="color:')
           .append(getOddsFgColor(this.plaColorInd[i])).append(';background-color:')
           .append(getOddsBgColor(this.plaColorInd[i]))
           .append('" href="javascript:processQuickBet(\'PLA\', \'' + this.raceNo + '\', ' + i + ')"><nobr>' + this.plaOdds[i] + '</nobr></a>');
      else
        buf.append('<span class="wpTdColor" style="color:')
           .append(getOddsFgColor(this.plaColorInd[i])).append(';background-color:')
           .append(getOddsBgColor(this.plaColorInd[i]))
           .append('"><nobr>').append(this.plaOdds[i]).append('</nobr></span>');
      ...
      if ( enableBS ) {
        ...
        if ( disabled )
          this.selectLeg['W-P'][i] = 0;
        buf.append('<input type="checkbox" class="checkbox" onclick="mExcludeChkbox(')
           .append(this.raceNo).append(', ').append(i).append(', \'W-P\');" ');
        buf.append((this.selectLeg['W-P'][i]==1) ? 'checked ' : '');
        buf.append(disabled ? 'disabled' : '').append('>');
      }
...
function DblTable(r1, r2) {
  ...
  this.setTableToDiv = function() {
    if ( this.fieldSize1 > 14 )
      document.getElementById('pagingDBL').innerHTML = genPagingDBL();
  ...
  this.generateTable = function() {
    ...
    for ( var i=1; i<=14; i++ ) {
      if ( (this.curPage-1)*14+i <= 24 )
        buf.append('<td width="40" rowspan="2" align="center" class="tdColor1">').append(((this.curPage-1)*14+i)).append('</td>');
      else
        buf.append('<td width="40" rowspan="2" align="center" class="tdColor1">&nbsp;</td>');
    }
    buf.append('</tr>');
    buf.append('<tr>');
    ...
    for ( var i=1; i<=rowCnt; i++ ) {
      ...
      for ( var j=1; j<=14; j++ ) {
        ...
        if ( !this.haveOdds || i>this.fieldSize2 || xAxis>this.fieldSize1 )
          buf.append('');
        else {
          ...
          if ( this.dblOdds[xAxis][i]=='SCR' || (n1&&s2) || (n2&&s1) || (n1&&sr2) || (n2&&sr1)
            || (s1&&sr2) || (s2&&sr1) || (s1&&s2) || (sr1&&sr2) )
            buf.append('<nobr>').append(scratchLbl).append('</nobr>');
          else if ( r1 || r2 )
            buf.append('<span class="wpTdColor"><nobr>---</nobr></span>');
          else if ( this.dblOdds[xAxis][i]!=null && ranRace<this.race1 && this.startSell )
            buf.append('<a class="tdColor" style="color:')
               .append(getOddsFgColor(this.dblColorInd[xAxis][i])).append(';background-color:')
               .append(getOddsBgColor(this.dblColorInd[xAxis][i]))
               .append('" href="javascript:processQuickBet(\'DBL\', \'' + this.race1 + '\', ' + xAxis + ', ' + i + ')">' + this.dblOdds[xAxis][i] + '</a>');
          else
            buf.append('<span class="tdColor" style="color:')
               .append(getOddsFgColor(this.dblColorInd[xAxis][i])).append(';background-color:')
               .append(getOddsBgColor(this.dblColorInd[xAxis][i]))
               .append('">').append(this.dblOdds[xAxis][i]).append('</span>');
        }
        buf.append('</td>');
      }
      buf.append('</tr>');
...
function MultiLegTable(rNo) {
  ...
  this.setTableToDiv = function() {
    ...
    switch ( this.curFunc ) {
      ...
      case 1 :
        if ( this.tableType=='T' )
          document.getElementById('paging' + this.selectPool).innerHTML = genPaging(this.selectPool);
        break;
      default :
        document.getElementById('paging' + this.selectPool).style.display = 'none';
    }
  ...
  this.generateTable = function() {
    if ( this.tableType=='Q' )
	      return this.generateTableQ();
    if ( this.tableType=='T' )
          if (this.curFunc == 4)
            return this.generateTableInvest();
          else
    		return this.generateTableT();
    return '';
  }
  ...
  this.generateTableT = function() {
    ...
    if ( this.curFunc==2 || this.curFunc==3 )
      buf.append(' ').append(this.theNthPageLbl.replace('#', display_number[this.curPage]))
    buf.append('</td></tr>');
    if ( this.curFunc==1 ) {
      buf.append('<tr>');
      for ( var j=0; j<4; j++ ) {
        ...
        if ( this.tableObj[startIdx+j]!=null && this.tableObj[startIdx+j][0].hName!=null ) {
          ...
          if ( !isOverseaMeeting )
			...
			buf.append('<a href="javascript:WACommonTagging(\'horse\');goHorseRecord2(\'').append(jcewUrlLbl)
               .append('\', \'').append(this.tableObj[startIdx+j][0].hCode).append('\');">');
			// END Nielsen Online SiteCensus

          for ( var i=0; i<this.tableObj[startIdx+j].length; i++ ) {
            if ( i > 0 )
              buf.append(' /<br>');
            if ( isOverseaMeeting && this.tableObj[startIdx+j][i].hCode!='' )
              buf.append('<nobr>(').append(this.tableObj[startIdx+j][i].hCode).append(')</nobr>');
            if ( this.tableObj[startIdx+j][i].hName!='' )
              buf.append(this.tableObj[startIdx+j][i].hName);
            else
              buf.append('-');
            if ( this.tableObj[startIdx+j][i].hStat.indexOf('R')>=0 )
              buf.append('<nobr>(').append(reserveLbl).append(')</nobr>');
            if ( this.tableObj[startIdx+j][i].hStat.indexOf('S')>=0 )
              buf.append('<nobr>(').append(scratchLbl).append(')</nobr>');
          }
          if ( !isOverseaMeeting )
            buf.append('</a>');
    ...
    for ( var i=0; i<this.rowSize; i++ ) {
      ...
      for ( var j=0; j<4; j++ ) {
        ...
        buf.append('<td width="17%" class="').append(this.tbClass[i%2])
           .append('" style="border-right:1px solid #FFFFFF;"><nobr><strong>&nbsp;');
        if ( hideReserve || hideScratch )
          buf.append('');
        else
          buf.append(this.selTable[i][j]);
        buf.append('</strong></nobr></td>');

        buf.append('<td width="8%" class="').append(this.tbClass[i%2])
           .append('" style="border-right:1px solid #CCCCCC;" align="center">');
        if ( hideReserve || hideScratch )
          buf.append('');
        else if ( this.oddsTable[i][j]!=null && this.oddsTable[i][j]!='SCR' && this.oddsTable[i][j]!='---'
                  && ranRace<this.raceNo && this.startSell )
          buf.append('<a href="javascript:processQuickBet(\'')
             .append(this.selectPool).append('\', \'').append(this.raceNo).append('\', \'')
             .append(this.selTable[i][j]).append('\')" class="tableNum">')
             .append(this.oddsTable[i][j]).append('</a>');
        else
          buf.append(this.oddsTable[i][j]);
        buf.append('</td>');
      }
      buf.append('</tr>');
    }
  ...
  this.generateTableInvest = function() {
      var buf = new StringBuffer();
      buf.append('<table width="100%" border="0" cellspacing="0" cellpadding="2" bgcolor="#DDDCDC" style="border:1px solid #CCCCCC;">');
      buf.append('<tr><td height="25" align="center" class="tdColor1" colspan="4">').append(this.header).append('</td></tr>');
      //Header
      buf.append('<tr>');
      ...
      for (var i = 0; i < this.rowSize; i++) {
          ...
          for (var j = 1; j < 4; j++) {
              buf.append('<td width="*" class="').append(this.tbClass[i % 2])
           .append('" style="border-right:1px solid #CCCCCC;" align="center">');
              var displayVal = '';
              if (this.oddsTable[i][j] == 'SCR')
                  displayVal = scratchLbl;
              else if (this.oddsTable[i][j] == '---')
                  displayVal = reserveLbl;
              else if (this.oddsTable[i][j] == '------')
                  displayVal = ""; //show nothing for overflow
              else if (!(this.oddsTable[i][j] === ""))
                  displayVal = '$' + addComma(this.oddsTable[i][j]);
  ...
  this.generateTableQ = function() {
    ...
    buf.append('<tr>');
    buf.append('<td width="56" colspan="3" class="tdColorHead" style="PADDING-LEFT:5px;border:1px solid #dddcdc"><strong>')
       .append(this.header).append('</strong></td>');
    for ( var i=2; i<=14; i++ )
      buf.append('<td width="18" align="center" class="tdColorNum1" style="border-top:1px solid #dddcdc;border-right:1px solid #dddcdc;border-bottom:1px solid #dddcdc;">').append(i).append('</td>');
    ...
    for ( var i=0; i<7; i++ ) {
      ...
      for ( var j=8; j<x; j++ ) {
        ...
        if ( !this.haveOdds || j>this.qFieldSize || x>this.qFieldSize )
          buf.append('&nbsp;');
        else {
          // s1 = 1st leg scratch; s2 = 2nd leg scratch
          // r1 = 1st leg reserve; r2 = 2nd leg reserve
          ...
          if ( this.qOdds[j][x]=='SCR' || (n1&&s2) || (n2&&s1) || (n1&&sr2) || (n2&&sr1)
            || (s1&&sr2) || (s2&&sr1) || (s1&&s2) || (sr1&&sr2) )
            buf.append(scratch2Lbl);
          else if ( r1 || r2 )
            buf.append('-');
          else if ( this.qOdds[j][x]!=null && ranRace<this.raceNo && this.startSell )
            buf.append('<a class="tdColor" style="color:')
               .append(getOddsFgColor(this.qColorInd[j][x])).append(';background-color:')
               .append(getOddsBgColor(this.qColorInd[j][x]))
               .append('" href="javascript:processQuickBet(\'').append(this.selectPool)
               .append('\', \'' + this.raceNo + '\', \'' + j + '-' + x + '\')">' + this.qOdds[j][x] + '</a>');
          else
            buf.append('<span class="tdColor" style="color:')
               .append(getOddsFgColor(this.qColorInd[j][x])).append(';background-color:')
               .append(getOddsBgColor(this.qColorInd[j][x]))
               .append('">').append(this.qOdds[j][x]).append('</span>');
        }
        buf.append('</td>');
      }
      ...
      for ( var j=y+1; j<=14; j++ ) {
        buf.append('<td align="center" width="18" class="tdColorNum2" style="border-right:1px solid #dddcdc;border-bottom:1px solid #dddcdc;">');
        if ( !this.haveOdds ||  y>this.qFieldSize || j>this.qFieldSize )
          buf.append('&nbsp;');
        else {
          // s1 = 1st leg scratch; s2 = 2nd leg scratch
          // r1 = 1st leg reserve; r2 = 2nd leg reserve
          ...
          if ( this.qOdds[y][j]=='SCR' || (n1&&s2) || (n2&&s1) || (n1&&sr2) || (n2&&sr1)
            || (s1&&sr2) || (s2&&sr1) || (s1&&s2) || (sr1&&sr2) )
            buf.append(scratch2Lbl);
          else if ( r1 || r2 )
            buf.append('-');
            ...
            buf.append('<a class="tdColor" style="color:')
               .append(getOddsFgColor(this.qColorInd[y][j])).append(';background-color:')
               .append(getOddsBgColor(this.qColorInd[y][j]))
               .append('" href="javascript:processQuickBet(\'').append(this.selectPool)
               .append('\', \'' + this.raceNo + '\', \'' + y + '-' + j + '\')">' + this.qOdds[y][j] + '</a>');
          else
            buf.append('<span class="tdColor" style="color:')
               .append(getOddsFgColor(this.qColorInd[y][j])).append(';background-color:')
               .append(getOddsBgColor(this.qColorInd[y][j]))
               .append('">').append(this.qOdds[y][j]).append('</span>');
        }
        buf.append('</td>');
      }
  ...
  this.generateTableQ24 = function() {
    ...
    for ( var j=15; j<=24; j++ )
      buf.append('<td width="18" align="center" class="tdColorNum1" style="border-top:1px solid #dddcdc;border-right:1px solid #dddcdc;border-bottom:1px solid #dddcdc;">').append(j).append('</td>');
    buf.append('<td width="18" align="center" class="tdColorNum1" style="border-top:1px solid #dddcdc;border-right:1px solid #dddcdc;border-bottom:1px solid #dddcdc;">&nbsp;</td>');
    buf.append('</tr>');
    ...
    for ( var i=1; i<=23; i++ ) {
      ...
      for ( var j=15; j<=24; j++ ) {
        ...
        if ( !this.haveOdds || this.qOdds[i][j]=='' || i>this.qFieldSize || j>this.qFieldSize )
          buf.append('&nbsp;');
        else {
          // s1 = 1st leg scratch; s2 = 2nd leg scratch
          // r1 = 1st leg reserve; r2 = 2nd leg reserve
          ...
          if ( this.qOdds[i][j]=='SCR' || (n1&&s2) || (n2&&s1) || (n1&&sr2) || (n2&&sr1)
            || (s1&&sr2) || (s2&&sr1) || (s1&&s2) || (sr1&&sr2) )
            buf.append(scratch2Lbl);
          else if ( r1 || r2 )
            buf.append('-');
            ...
            buf.append('<a class="tdColor" style="color:')
               .append(getOddsFgColor(this.qColorInd[i][j])).append(';background-color:')
               .append(getOddsBgColor(this.qColorInd[i][j]))
               .append('" href="javascript:processQuickBet(\'').append(this.selectPool)
               .append('\', \'' + this.raceNo + '\', \'' + i + '-' + j + '\')">' + this.qOdds[i][j] + '</a>');
          else
            buf.append('<span class="tdColor" style="color:')
               .append(getOddsFgColor(this.qColorInd[i][j])).append(';background-color:')
               .append(getOddsBgColor(this.qColorInd[i][j]))
               .append('">').append(this.qOdds[i][j]).append('</span>');
        }
        buf.append('</td>');
      }
...
function JkcTable() {
  ...
  this.getJockeyNameByNum = function(sNum) {
    for ( var i=1; i<this.jockeyNameList.length; i++ ) {
      if ( sNum==this.jockeyNumList[i] )
        return this.jockeyNameList[i].replace('\\\'', '\'');
    }
    return '';
  }
  ...
  this.generateTable = function() {
      ...
      if (this.displayJkcPts)
          buf.append('<td class="tableContentHead" align="center" width="15%">').append(this.lblJkcPts).append('</td>');
      if (this.displayScheduleRides)
          buf.append('<td class="tableContentHead" align="center" width="15%">').append(this.lblScheduleRides).append('</td>');
      if (this.displayRemainingRides)
          buf.append('<td class="tableContentHead" align="center" width="15%">').append(this.lblRemainingRides).append('</td>');
      if (this.displayOpOdds)
          buf.append('<td class="tableContentHead" align="center" width="15%">').append(this.lblOpOdds).append('</td>');
      if (this.displayPrevOdds)
          buf.append('<td class="tableContentHead" align="center" width="15%">').append(this.lblPrevOdds).append('</td>');
      ...
      for (var i = 1; i < this.jockeyNameList.length; i++) {
          ...
          buf.append('<td class="').append(tbStyle[i % 2]).append('">').append('<a href="javascript:WACommonTagging(\'jockey\');goJockeyRecord2(\'')
         .append(jcewUrlLbl).append('\', \'').append(this.jockeyCodeList[i]).append('\', ').append(this.jockeyNumList[i] == this.otherSelNo).append(');">')
         .append('<span class="hkscsfontfamily">').append(this.jockeyNameList[i].replace('\\\'', '\'')).append('</span>')
         .append('</a>').append('</td>');
          // END Nielsen Online SiteCensus

          if (this.displayJkcPts)
              buf.append('<td align="center" class="').append(tbStyle[i % 2]).append('">').append(this.jkcPts[i] == '-1' ? '---' : this.jkcPts[i]).append('</td>');
          ...
          if (otherDetailEnable && this.jockeyNumList[i] == this.otherSelNo) {
              ...
              .append("generateOtherDetailTable(")
              .append("stage, ")
              .append("'").append(this.lblOtherJockey).append("', ")
              .append("'").append(this.lblOtherPoint).append("', ")
              .append("'").append(this.lblScheduleRides).append("', ")
              .append("'").append(this.lblRemainingRides).append("', jcewUrl, jkcOtherData, false)")
              .append(", OFFSETX, ").append(stage == 5 ? "-300" : "-220")
              .append(", ABOVE, true, BORDERWIDTH, 0, BGCOLOR, '', PADDING, 0, CLICKCLOSE, true, CLICKSTICKY, true, FOLLOWMOUSE, false, DELAY, 0)\" id=\"jkcDetail\">")
              .append(this.lblOtherDetail)
              .append("</a>");
          }

          if (this.displayScheduleRides)
              buf.append('<td align="center" class="').append(tbStyle[i % 2]).append('">').append(otherDetailEnable && this.jockeyNumList[i] != this.otherSelNo ? this.jkcScheduleRides[i] : otherDetailBuf.toString()).append('</td>');

          if (this.displayRemainingRides)
              buf.append('<td align="center" class="').append(tbStyle[i % 2]).append('">').append(otherDetailEnable && this.jockeyNumList[i] != this.otherSelNo ? this.jkcRemainingRides[i] : otherDetailBuf.toString()).append('</td>');

          if (this.displayOpOdds) {
              buf.append('<td align="center" class="').append(tbStyle[i % 2]).append('">');
              if (this.jkcRefOdds[i] == 'LSE')
                  buf.append('<nobr>').append(this.lblLse).append('</nobr>');
              else if (this.jkcRefOdds[i] == 'RFD')
                  buf.append('<nobr>').append(this.lblRfd).append('</nobr>');
              else
                  buf.append(this.jkcRefOdds[i])
              buf.append('</td>');
          }
          if (this.displayPrevOdds) {
              buf.append('<td align="center" class="').append(tbStyle[i % 2]).append('">');
              if (this.jkcPrevOdds[i] == 'LSE' || this.joStatus[i] == '2')
                  buf.append('<nobr>').append(this.lblLse).append('</nobr>');
              else if (this.jkcPrevOdds[i] == 'RFD' || this.joStatus[i] == '3')
                  buf.append('<nobr>').append(this.lblRfd).append('</nobr>');
              else
                  buf.append(handleEmptyStr(this.jkcPrevOdds[i]));
              buf.append('</td>')
          }
          ...
          if (this.jkcLastOdds[i] == 'LSE' || this.joStatus[i] == '2')
              buf.append('<nobr>').append(this.lblLse).append('</nobr>');
          else if (this.jkcLastOdds[i] == 'RFD' || this.joStatus[i] == '3')
              buf.append('<nobr>').append(this.lblRfd).append('</nobr>');
          else if (this.startSell == 1 && this.joStatus[i] == '1' && this.sellStatus[i] == '1')
              buf.append('<a class="wpTdColor" style="color:#000000" href="javascript:processQuickBet(\'JKC\', \'')
           .append(this.jockeyNumList[i]).append('@').append(this.jkcLastOdds[i]).append('\')">').append(this.jkcLastOdds[i]).append('</a>');
          else
              buf.append((this.sellStatus[i] == '1' && isNumeric(this.jkcLastOdds[i])) ? this.jkcLastOdds[i] : '---');
...
function jkcLegClick(i) {
  if ( jTable.selectLeg[i]!=1 )
    jTable.selectLeg[i] = 1;
  else
    jTable.selectLeg[i] = 0;
